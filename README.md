...
```text
book/                         Пособие в PDF и исходники LaTeX
...
downloads/                    Архив исходников для Overleaf
START_HERE.md                 Инструкция первого запуска
CITATION.cff                  Авторы и сведения для цитирования
```

Исходники книги собираются из `book/main.tex` компилятором **pdfLaTeX**. Для Overleaf загрузите [архив проекта](downloads/itmo_practicum_v10_overleaf.zip).

## Преподавателю: добавить данные

Поместите файлы в `data/` соответствующей работы. **Пути в ноутбуках менять не требуется.** Для первых двух работ нужны отдельные пары `train.csv` и `test.csv`; для третьей — пять каталогов изображений. Точная структура описана в каждой папке данных.

Рядом сохраните источник и описание признаков, дату или версию выгрузки. Студенты должны использовать одну согласованную версию. Папки данных доступны для добавления в Git; результаты экспериментов и веса исключены через `.gitignore`.

## Версия и воспроизводимость

Комплект **v0.10 · 20 сентября 2026**. [Зафиксированная версия учебных материалов](https://github.com/stalkim/itmo-ml-practicum/tree/v0.10) содержит пособие, код и ноутбуки. Выборки преподаватель добавляет отдельно; наличие данных проверяйте в основной ветке.
...
Проверки учебного кода на небольших диагностических примерах описаны в [заметках о среде](code/README.md). Полный запуск на выборках курса ещё предстоит проверить после их добавления.
